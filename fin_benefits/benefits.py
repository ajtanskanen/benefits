...
class Benefits():
    ...
    def toimeentulotuki_lapsiosuus(self,lapset_alle10,lapset_10_17,lapset_18,lapsiparam):
        if lapset_alle10<1: 
            tuki = 0
        elif lapset_alle10==1:
            tuki = lapsiparam[0,0]
        elif lapset_alle10==2:
            tuki = lapsiparam[0,0]+lapsiparam[0,1]
        else:
            tuki = lapsiparam[0,0]+lapsiparam[0,1]+lapsiparam[0,2]*(lapset_alle10-2)
        
        if lapset_10_17==1:
            tuki += lapsiparam[1,0]
        elif lapset_10_17==2:
            tuki += lapsiparam[1,0]+lapsiparam[1,1]
        else:
            tuki += lapsiparam[1,0]+lapsiparam[1,1]+lapsiparam[1,2]*(lapset_10_17-2)

        if lapset_18==1:
            tuki += lapsiparam[2,0]
        elif lapset_18==2:
            tuki += lapsiparam[2,0]+lapsiparam[2,1]
        else:
            tuki += lapsiparam[2,0]+lapsiparam[2,1]+lapsiparam[2,2]*(lapset_18-2)
    ...
    def toimeentulotuki(self,omabruttopalkka: float,omapalkkavero: float,puolison_bruttopalkka: float,puolison_palkkavero: float,
                             ...
                             p: dict,omavastuuprosentti: float=0.0,alennus: int=0,asumistuki: float=None,lapsilisa: float=None,muut_tulot_asumistuki: float=None):
    ...
    def yleistuki2026(self,lapsia: int) -> float:
        '''
        params for yleistuki2026
        
        :param self: Description
        :param lapsia: Description
        :type lapsia: int
        :return: Description
        :rtype: float
        '''
    ...
    def yleistuki2027(self,lapsia: int) -> float:
        if self.use_extra_ppr:
            pvraha=21.5*37.21*1.015*self.extra_ppr_factor
        else:
            pvraha=21.5*37.21*1.015
        tuki=max(0,pvraha)    
    ...
    def elaketulovahennys2026(self,elaketulot: float,puhdas_ansiotulo: float):
        '''
        päivitetty 29.1.2025
        '''
        raja1_elaketulovahennys_kunnallis=11_030/self.kk_jakaja
    ...
    def tyotulovahennys2023(self,ika: float,lapsia: int,yksinhuoltaja: int):
        if ika>=60:
            if ika>=62:
    ...
    def tyotulovahennys2024(self,ika: float,lapsia: int,yksinhuoltaja: int):
        if ika>=65:
    ...
    def tyotulovahennys2025(self,ika: float,lapsia: int,yksinhuoltaja: int=0):
        '''
    ...
    def tyotulovahennys2026(self,ika: float,lapsia: int,yksinhuoltaja: int=0):
        '''
        tarvitaan tieto yksihuoltajuudesta ja iästä, FIXME!
        päivitetty 29.1.2025
        '''
    ...
    def laske_tyotulovahennys2018_2022(self,puhdas_ansiotulo: float,palkkatulot_puhdas: float,ika: float, lapsia: int,yksinhuoltaja: int=0):
        '''
        Vuosille 2018-2022
        '''
    ...
    def laske_tyotulovahennys2023_2026(self,puhdas_ansiotulo: float,palkkatulot_puhdas: float,ika: float,lapsia: float,yksinhuoltaja: int=0):
        ...
        Vuosille 2023-
        '''
        max_tyotulovahennys,ttulorajat,ttulopros=self.tyotulovahennys(ika,lapsia,yksinhuoltaja)
    ...
    def ansiotulovahennys2026(self):
        '''
        Ei käytössä 2025 alkaen
        '''
        rajat=np.array([2500,7230,14000])/self.kk_jakaja
    ...
    def veroparam2022(self):
        ...
        Päivitä
        '''
        self.kunnallisvero_pros=max(0,0.2000+self.additional_kunnallisvero) 
        self.kirkollisvero_pros=0.0139
    ...
    def veroparam2023(self):
        ...
        Päivitetty 6.5.2023
        '''
        self.kunnallisvero_pros=max(0,0.0743+self.additional_kunnallisvero) # +0.0135) 
    ...
    def laske_ylevero2026(self,puhdas_ansiotulo: float):
        yleveropros=0.025
        min_ylevero=0/self.kk_jakaja
        max_ylevero=163/self.kk_jakaja
        ylevero_alaraja=14000/self.kk_jakaja
    ...
    def perusvahennys2025(self):
        perusvahennys_pros=0.18
        max_perusvahennys=4_115/self.kk_jakaja
        return perusvahennys_pros,max_perusvahennys
    ...
    def verotus(self,palkkatulot: float,muuttulot: float,elaketulot: float,lapsia: int,p: dict,alku: str='',yksinhuoltaja: int=0):
        ...
        tyotulovahennys=self.laske_tyotulovahennys(puhdas_ansiotulo,palkkatulot_puhdas,p['ika'],lapsia,yksinhuoltaja)

        # perusvähennys
        if self.year>=2025:
            ansiotulovahennys_valtio=0
            perusvahennys_valtio=self.laske_perusvahennys(tulot_valtio)
        elif self.year>=2023:
...
### Hoitolisä
    ...
    def hoitolisa2026(self,perheentulot: float,perhekoko: int):
        '''
        Vuoden 2025 tasossa
        '''
        perustuki=202.12
...
# joustava hoitoraha
    ...
    def joustavahoitoraha(self,työaika: float,allekolmev: int):
        '''
        osatuki alle 22,5 h/vko
        päivitetty vuoteen 2025
        '''
        if allekolmev<1:
            tuki=0
        else:
...
# kotihoidontuki
    ...
    def kotihoidontuki2026(self,lapsia: int,allekolmev: int,alle_kouluikaisia: int) -> float:
        '''
        vuoden 2025 tasossa
        '''
        if lapsia<1:
    ...
    def laske_valtionvero2024_2025(self,tulot: float,p: dict) -> float:
        '''
        Sote-alueiden vuoksi kunnallisveroa siirrettiin valtionveroon. Rakennemuutos
        '''
        rajat,pros=self.valtionvero_asteikko()
    ...
    def lapsilisa2026(self,yksinhuoltajakorotus: bool=False,alle3v: int=0) -> float:
        '''
        vuoden 2025 tasossa
    ...
    def opiskelija_asumislisa(self,palkkatulot,kokoelake,asumismenot_asumistuki,aikuisia,kuntaryhma,p):
        if self.year>=2025:
            if kuntaryhma in set([1,2]):
                tuki = 296
            elif kuntaryhma==3:
                tuki = 248
            elif kuntaryhma==4:
                tuki = 216
            return tuki * aikuisia
        else:
            return 0
    ...
    def check_p(self,p: dict):
        ...
        for alku in set(['omat_','puoliso_','']):
            ...
            if alku+'tyoelake' not in p:
                p[alku+'tyoelake']=0
            if alku+'tyokyvyttomyyselake' not in p:
                p[alku+'tyokyvyttomyyselake']=0
            if alku+'osittainenvanhuuselake' not in p:
                p[alku+'osittainenvanhuuselake']=0
            if alku+'vanhuuselake' not in p:
                p[alku+'vanhuuselake']=0
    ...
    def setup_puoliso_q(self,p: dict,q: dict,puoliso: str='puoliso_',alku: str='puoliso_',
        ...
        q[puoliso+'tyokyvyttomyyselake']=0
        q[puoliso+'osittainenvanhuuselake']=0
        q[puoliso+'vanhuuselake']=0
        ...
        if p['aikuisia']>1 and p[puoliso+'alive']>0:
            ...
            if p[alku+'elakkeella']>0: # vanhuuseläkkeellä
                ...
                q[puoliso+'tyokyvyttomyyselake']=p[alku+'tyokyvyttomyyselake']
                q[puoliso+'osittainenvanhuuselake']=p[alku+'osittainenvanhuuselake']
                q[puoliso+'vanhuuselake']=p[alku+'vanhuuselake']
    ...
    def setup_omat_q(self,p: dict,omat: str='omat_',alku: str='',include_takuuelake: bool=True,add_kansanelake: bool=True):
        ...
        q[omat+'tyoelake']=0
        q[omat+'tyokyvyttomyyselake']=0
        q[omat+'osittainenvanhuuselake']=0
        q[omat+'vanhuuselake']=0
        ...
        if p[alku+'alive']<1:
            ...
            q[omat+'tyokyvyttomyyselake']=p[alku+'tyokyvyttomyyselake']
            q[omat+'osittainenvanhuuselake']=p[alku+'osittainenvanhuuselake']
            q[omat+'vanhuuselake']=p[alku+'vanhuuselake']
    ...
    def laske_tulot_v3(self,p: dict,tt_alennus=0,include_takuuelake: bool=True,omat: str='omat_',omatalku: str='',puoliso: str='puoliso_',puolisoalku: str='puoliso_',
        ...
        - lisätty työtuloon yksihuoltajakorotus
        '''
        self.check_p(p)
        ...
        _,q[omat+'verot'],q[omat+'valtionvero'],q[omat+'kunnallisvero'],q[omat+'kunnallisveroperuste'],q[omat+'valtionveroperuste'],\
            ...
            self.verotus(q[omat+'palkkatulot'],q[omat+'tyotpvraha']+q[omat+'aitiyspaivaraha']+q[omat+'isyyspaivaraha']\
                ...
                q[omat+'kokoelake'],lapsia,p,alku=omatalku,yksinhuoltaja=yksinhuoltaja)
        _,q[omat+'verot_ilman_etuuksia'],_,_,_,_,_,_,_,_,_,_,_,_,_=self.verotus(p['t'],0,0,lapsia,p,alku=omatalku,yksinhuoltaja=yksinhuoltaja)
        if q[omat+'kokoelake']>0:
            _,q[omat+'verot_vain_elake'],_,_,_,_,_,_,_,_,_,_,_,_,_=self.verotus(0,0,q[omat+'kokoelake'],lapsia,p,alku=omatalku,yksinhuoltaja=yksinhuoltaja)
        else:
        ...
        if aikuisia>1 and p[puoliso+'alive']>0:
            ...
            q[puoliso+'ptel'],q[puoliso+'sairausvakuutusmaksu'],q[puoliso+'tyotvakmaksu'],q[puoliso+'tyel_kokomaksu'],q[puoliso+'ylevero']=\
                self.verotus(q[puoliso+'palkkatulot'],
                    ...
                    q[puoliso+'kokoelake'],0,p,alku=puoliso,yksinhuoltaja=0) # onko oikein että lapsia 0 tässä????
            _,q[puoliso+'verot_ilman_etuuksia'],_,_,_,_,_,_,_,_,_,_,_,_,_=self.verotus(q[puoliso+'palkkatulot'],0,0,0,p,alku=puoliso,yksinhuoltaja=0)
            if q[puoliso+'kokoelake']>0:
                _,q[puoliso+'verot_vain_elake'],_,_,_,_,_,_,_,_,_,_,_,_,_=self.verotus(0,0,q[puoliso+'kokoelake'],lapsia,p,alku=omatalku,yksinhuoltaja=0)
            else:
        ...
        elif (((p[omatalku+'opiskelija']>0 and aikuisia<2) or (p[omatalku+'opiskelija']>0 and p[puolisoalku+'opiskelija']>0) and aikuisia==2)) and self.year>=2025: # opiskelijat asumislisän piiriin
            q['asumistuki']=self.opiskelija_asumislisa(q['palkkatulot'],q['kokoelake'],p['asumismenot_asumistuki'],aikuisia,p['kuntaryhma'],p)
    ...
    def update_toimeentulotuki(self,p: dict,q: dict,omat: str,puoliso: str,omatalku: str,puolisoalku: str):
        muut_tulot_asumistuki = q['tyotpvraha']+q['aitiyspaivaraha']+q['isyyspaivaraha']+q['kotihoidontuki']+q['sairauspaivaraha']+q['opintotuki']+q['kokoelake']
            ...
            if p['aikuisia']<2:
                if p[omatalku+'opiskelija']>0 or p[omatalku+'alive']<1:
                    ...
                    q['toimeentulotuki']=self.toimeentulotuki(p[omatalku+'t'],q[omat+'verot_ilman_etuuksia'],0,0,\
                        ...
                        0,p['asumismenot_toimeentulo'],q['pvhoito'],p['aikuisia'],p['lapsia'],p['kuntaryhma'],p,asumistuki=q['asumistuki'],muut_tulot_asumistuki=muut_tulot_asumistuki)
            else:
    ...
    def laske_alv(self,kateen: float):
        ...
        alv=(self.arvonlisavero+self.additional_vat)/(1.0+self.arvonlisavero+self.additional_vat)
    ...
    def asumistuki2026(self,palkkatulot1: float,palkkatulot2: float,muuttulot: float,vuokra: float,aikuisia: int,lapsia: int,kuntaryhma: int,p: dict) -> float:
        # Ruokakunnan koko
        # henkilöä    I kuntaryhmä,
        # e/kk    II kuntaryhmä,
        # e/kk    III kuntaryhmä,
    ...
    def elakkeensaajan_asumistuki_2025(self,palkkatulot: float,muuttulot: float,vuokra: float,aikuisia: int,kuntaryhma: int,p: dict,puolisolla_oikeus: bool=False) -> float:
        ...
        lisaomavastuu=0.435*max(0,palkkatulot+muuttulot-tuloraja)
    ...
    def elakkeensaajan_asumistuki_2026(self,palkkatulot: float,muuttulot: float,vuokra: float,aikuisia: int,kuntaryhma: int,p: dict,puolisolla_oikeus: bool=False) -> float:
        # Ruokakunnan koko
        # henkilöä    I kuntaryhmä,
        # e/kk    II kuntaryhmä,
        # e/kk    III kuntaryhmä,
    ...
    def paivahoitomenot2026(self,hoidossa: int,tulot: float,aikuisia: int, lapsia: int,p: dict,prosentti1: float=None,prosentti2: float=None,prosentti3: float=None,maksimimaksu: float=None):
        '''
        Päivähoitomaksut 1.8.2021
        '''
        minimimaksu=27
    ...
    def laske_kansanelake2025(self,ika: int,tyoelake: float,yksin: int,disability: bool=False,lapsia: int=0) -> float:
        ...
        if disability:
            maara = max(0,maara-np.maximum(0,(tyoelake-66.29))/2)
        else:
            if ika>=65:
                maara = max(0,maara*(1.0+0.072*(ika-65))-np.maximum(0,(tyoelake-66.29))/2)
            elif ika>=62: # varhennus
                maara = max(0,maara*(1.0-0.048*(65-ika))-np.maximum(0,(tyoelake-66.29))/2)
            else:
                maara=0
    ...
    def laske_kansanelake2026(self,ika: int,tyoelake: float,yksin: int,disability: bool=False,lapsia: int=0) -> float:
        '''
        Päivitä
        '''
        if yksin>0:
    ...
    def laske_takuuelake2025(self,ika: int,muuelake: float,disability: bool=False,lapsia: int=0) -> float:
        ...
        lapsikorotus = 26.18*lapsia
        
        if muuelake < 986.30+lapsikorotus:
            elake = 986.30+lapsikorotus-muuelake
        else:
            elake = 0
    ...
    def laske_takuuelake2026(self,ika: int,muuelake: float,disability: bool=False,lapsia: int=0) -> float:
        if ika<63 and not disability:
            return 0
    ...
    def set_year(self,vuosi: int):
        ...
        if vuosi==2019:
            ...
            self.laske_valtionvero=self.laske_valtionvero2024_2025
            ...
            self.isyysraha=self.isyysraha_perus
            self.peruspaivaraha=self.yleistuki2026
