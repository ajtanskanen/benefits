...
class UniversalCreditBenefits():
    ...
    def universalcredit(self,omabruttopalkka,puolison_bruttopalkka,muuttulot,p,omavastuuprosentti=0.0,alennus=0):
        '''
        Esimerkki universal creditistä, joka korvaa kotihoidontuen, asumistuen, toimeentulotuen,
        ...
        suojaosa=300
        bruttopalkka=max(0,omabruttopalkka-suojaosa)+max(0,puolison_bruttopalkka-suojaosa)
        ...
        tuki=max(0,tuki1-(bruttopalkka+muuttulot)*0.5)
    ...
    def ansiopaivaraha(self,tyoton,vakiintunutpalkka,lapsia,tyotaikaisettulot,saa_ansiopaivarahaa,kesto,
            p,ansiokerroin=1.0,omavastuukerroin=1.0,alku=''):
    ...
    def peruspaivaraha(self,lapsia,palkka,p):
        if p['aikuisia']>1:
            return universalcredit(palkka,0,0,p,alennus=0.5) # puolet yhdelle henkilölle
        else:
            return universalcredit(palkka,0,0,p,alennus=0.0)

    def soviteltu_peruspaivaraha(self,lapsia,tyotaikaisettulot,ansiopvrahan_suojaosa,p):
        pvraha=self.peruspaivaraha(lapsia,tyotaikaisettulot,p)
...
#             print('verotus',d2-d1)
    ...
    def laske_tulot(self,p,tt_alennus=0,include_takuuelake=True,legacy=True):
        ...
        if p['elakkeella']>0:
            ...
            q['asumistuki']=0 #self.asumistuki(p['puoliso_tulot']+p['t'],q['ansiopvraha']+q['puoliso_ansiopvraha']+q['aitiyspaivaraha']+q['isyyspaivaraha']+q['kotihoidontuki']+q['sairauspaivaraha']+q['opintotuki'],p['asumismenot_asumistuki'],p)
        ...
        if p['elakkeella']>0:
            ...
            q['opintotuki_netto']=0 #q['opintotuki']-(q['verot']-q['verot_ilman_etuuksia'])
            ...
            q['kotihoidontuki_netto']=0 #q['kotihoidontuki']-(q['verot']-q['verot_ilman_etuuksia']) 
        elif p['sairauspaivarahalla']>0:
        ...
        if p['aikuisia']>1:
            if p['puoliso_tyoton']>0: # vanhuuseläkkeellä
                ...
                q['puoliso_opintotuki_netto']=0 #q['puoliso_opintotuki']-(q['puoliso_verot']-q['puoliso_verot_ilman_etuuksia'])
            elif p['puoliso_kotihoidontuella']>0:
                q['puoliso_kotihoidontuki_netto']=0 #q['puoliso_kotihoidontuki']-(q['puoliso_verot']-q['puoliso_verot_ilman_etuuksia']) 
        else:
            q['puoliso_ansiopvraha_netto']=0
        ...
        if p['opiskelija']>0:
            ...
            q['toimeentulotuki']=self.universalcredit()
